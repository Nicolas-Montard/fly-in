...
class Parser():
    ...
    def verif_nb_drone_line(self, line: str) -> None:
        ...
        if split_line[0] != "nb_drones":
            raise ParsingError("The first line isn't nb_drones: "
        ...
        try:
            ...
            raise ParsingError("The Value of nb_drone is invalid: "
                               f"{split_line[1]}", self.counter_line)
    ...
    def verif_hub_line(self, line: str) -> None:
        ...
        if len(values) != 4 and len(values) != 3:
            raise ParsingError(f"{len(values)} values found, 3 or 4 expected",
                               self.counter_line)
    ...
    def format_metadata(self, metadata: str) -> dict:
        if metadata[0] != "[" or metadata[-1] != "]":
            raise ParsingError("The metadata must begin and end with [], "
                                f"actual: {metadata}", self.counter_line)
    ...
    def verif_hub_name(self, name: str) -> None:
        if "-" in name:
            raise ParsingError(f"The name of the hub contain a dashes: {name}",
                               self.counter_line)
        if name in self.hub_names:
            raise ParsingError(f"This hub name is already defined: {name}",
                               self.counter_line)
        self.hub_names.add(name)

    def verif_hub_coord(self, x, y) -> None:
        try:
            x, y = int(x), int(y)
        except ValueError:
            raise ParsingError(f"Invalid coordonate: {x, y}",
                               self.counter_line)
        if (x, y) in self.hub_coord:
            raise ParsingError(f"Those coordinate are already defined: {x, y}",
                               self.counter_line)
    ...
    def split_line(self, line: str) -> list[str]:
        ...
        if len(split_line) <= 1:
            raise ParsingError("value isn't separated by ':'",
                               self.counter_line,)
